...
if st.sidebar.button("📥 Download Excel Template"):
    st.download_button(
        ...
        file_name="metro_model_template.xlsx",
...
if uploaded_file is not None:
    try:
        ...
        missing_cols = [col for col in feature_columns if col not in df_excel.columns]
        extra_cols = [col for col in df_excel.columns if col not in feature_columns]

        if missing_cols:
            st.error(f"Missing required columns: {missing_cols}")
            st.stop()
        if extra_cols:
            st.warning(f"These columns are extra and not used: {extra_cols}")
